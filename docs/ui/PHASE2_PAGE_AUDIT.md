...
- 首页：恢复关注入口、浅色校园图片宣传卡和大面积快递代拿 Hero（价格/截单/包裹视觉/整宽 CTA）、8 个本地图标、限制长图；品牌/搜索头部预留胶囊安全区，点赞收藏同步源数据与分栏，失败回滚和重复点击已通过行为回归。
...
- 本地 CI 与内存 Smoke：PASS。微信 CLI 实际返回 IDE service port disabled，编译与截图：NOT RUN。图标设备兼容性、胶囊避让、首页首屏比例和键盘安全区尚无截图验收。
