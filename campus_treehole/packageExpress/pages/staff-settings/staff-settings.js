...
Page({
  data: { campusId: '', saving: false, form: { acceptingOrders: false, basePriceCents: 300, pricingMode: 'PER_PACKAGE', baseOrderPriceCents: 0, perPackagePriceCents: 300, extraPickupPointPriceCents: 0, cutoffTime: '', deliveryWindow: '', notice: '', specialParcelNotice: '', parcelPricingJson: '', pickupJson: '', deliveryJson: '' } },
  onLoad() { this.load() },
  ...
  async onSave() {
    ...
    let deliveryCampuses = []; let pickupPoints = []; let parcelSizePricing = {}
    try { deliveryCampuses = JSON.parse(this.data.form.deliveryJson || '[]'); pickupPoints = JSON.parse(this.data.form.pickupJson || '[]'); parcelSizePricing = JSON.parse(this.data.form.parcelPricingJson || '{}') } catch (e) { return wx.showToast({ title: '配置 JSON 格式有误', icon: 'none' }) }
    this.setData({ saving: true })
    try {
      const res = await app.callDB('ownerUpdateExpressSettings', { acceptingOrders: this.data.form.acceptingOrders, basePriceCents: Number(this.data.form.basePriceCents), pricingMode: this.data.form.pricingMode, baseOrderPriceCents: Number(this.data.form.baseOrderPriceCents), perPackagePriceCents: Number(this.data.form.perPackagePriceCents), extraPickupPointPriceCents: Number(this.data.form.extraPickupPointPriceCents), parcelSizePricing, specialParcelNotice: this.data.form.specialParcelNotice, cutoffTime: this.data.form.cutoffTime, deliveryWindow: this.data.form.deliveryWindow, notice: this.data.form.notice, deliveryCampuses, pickupPoints })
