  ...
  if (!eventsModuleInstance) {
    eventsModuleInstance = createEventsModule({
      ...
      helpers: {
        ...
        resolveCampusIdForRead,
        DEFAULT_CAMPUS_ID,
        announcementTargetsCampus: (doc, campusId) => announcementTargetsCampus(doc, campusId),
...
/** 用户主页：TA 上架中的闲置商品 (由 modules/market.js 托管) */
...
// ---------- 活动专区（已模块化至 modules/events.js） ----------
async function fetchActivityZoneConfigDoc() {
  return await getEventsModule().fetchActivityZoneConfigDoc()
}

function sanitizeActivityZoneConfigForSet(doc) {
  return getEventsModule().sanitizeActivityZoneConfigForSet(doc)
}

async function persistActivityZoneConfig(doc) {
  return await getEventsModule().persistActivityZoneConfig(doc)
}

async function resolveActivityTagsForPost(campusId, category, existingPost = null) {
  return await getEventsModule().resolveActivityTagsForPost(campusId, category, existingPost)
}

async function convertActivityPostsToNormal(zoneDoc) {
  return await getEventsModule().convertActivityPostsToNormal(zoneDoc)
}

async function finalizeActivityZoneRound(triggeredByOpenid, reason = 'manual') {
  return await getEventsModule().finalizeActivityZoneRound(triggeredByOpenid, reason)
}

async function maybeAutoFinalizeActivityZone(zoneDoc) {
  return await getEventsModule().maybeAutoFinalizeActivityZone(zoneDoc)
}
...
async function endActivityZone(openid) {
  return await getEventsModule().endActivityZone(openid)
}
...
// ========== 集市操作 (已模块化至 modules/market.js) ==========

async function getMarketGoods(params) {
  return await getMarketModule().getMarketGoods(params)
}

async function getMarketGoodsById(goodsId, openid) {
  return await getMarketModule().getMarketGoodsById(goodsId, openid)
}

async function getMarketComments(goodsId) {
  return await getMarketModule().getMarketComments(goodsId)
}
...
async function toggleFavorGoods(openid, goodsId) {
  return await getMarketModule().toggleFavorGoods(openid, goodsId)
}

async function wantMarketGoods(openid, goodsId) {
  return await getMarketModule().wantMarketGoods(openid, goodsId)
}

async function deleteMarketGoods(openid, goodsId) {
  return await getMarketModule().deleteMarketGoods(openid, goodsId)
}
