...
Page({
  ...
  async loadExpressCard() {
    ...
    try {
      ...
      this.setData({ expressCard: { acceptingOrders: config.acceptingOrders === true, priceText, cutoffText, activeOrder: active ? { orderId: active._id, statusLabel: active.orderStatus === 'DELIVERING' ? '配送中' : '待取件', totalPackageCount: active.totalPackageCount || active.packageCount || 0, pickupPointCount: active.pickupPointCount || 1, summary: `${active.totalPackageCount || active.packageCount || 0}件 · ${active.pickupPointCount || 1}个快递点`, address: `${active.dormAreaNameSnapshot || active.dormArea || ''}${active.dormBuildingNameSnapshot || active.dormBuilding || ''} ${active.roomNumberSnapshot || active.roomNumber || ''}` } : null, serviceStatus: config.serviceStatus || (config.configured ? (config.acceptingOrders ? 'OPEN' : 'PAUSED') : 'UNCONFIGURED'), configured: config.configured === true } })
    } catch (e) {
