...
async function fixture(){
 const db=memoryDb(),users={},blocked=new Set();let clock=Date.UTC(2026,8,9,4),reject=false
 ...
 const heart=create({db,_:db.command,helpers,now:()=>clock,random:()=>0})
 async function add(id,gender='female',campusId='guit-hangtian'){
  ...
  const profile={enabled:true,adultDeclared:true,gender,interestedIn:['male','female','other'],photos:['cloud://test/photo.jpg'],bio:'摄影爱好者',interestIds:['摄影','羽毛球'],lookingFor:'周末散步',grade:'大二',allowFateCard:true}
  return {id,userId:publicId(id),profile,result:await heart.updateHeartProfile(id,profile)}
 }
 return {db,heart,users,blocked,add,setTime:t=>clock=t,setReject:r=>reject=r}
}
