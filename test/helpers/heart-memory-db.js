...
function memoryDb(){
 ...
 const matches=(row,where)=>Object.entries(where).every(([k,v])=>k==='$or'?v.some(q=>matches(row,q)):v&&v.$in?v.$in.includes(row[k]):v&&v.$all?v.$all.every(x=>(row[k]||[]).includes(x)):v&&v.$gte!==undefined?row[k]>=v.$gte:v&&v.$gt!==undefined?row[k]>v.$gt:v&&v.$lte!==undefined?row[k]<=v.$lte:v&&v.$lt!==undefined?row[k]<v.$lt:row[k]===v)
 ...
 where(q){let start=0,count=100,sort;const query={orderBy(k,d){sort=[k,d];return query},skip(n){start=n;return query},limit(n){count=n;return query},async count(){return {total:Object.values(rows()).filter(r=>matches(r,q)).length}},async get(){let found=Object.values(rows()).filter(r=>matches(r,q));if(sort)found.sort((a,b)=>(a[sort[0]]>b[sort[0]]?1:-1)*(sort[1]==='desc'?-1:1));return {data:clone(found.slice(start,start+count))}},async update({data}){let updated=0;Object.values(rows()).filter(r=>matches(r,q)).forEach(r=>{Object.assign(r,clone(data));updated++});return {stats:{updated}}},async remove(){let removed=0;Object.keys(rows()).forEach(id=>{if(matches(rows()[id],q)){delete rows()[id];removed++}});return {stats:{removed}}}};return query}
 }}}}
 const db=api(()=>store)
  db.startTransaction=async()=>{let release;const prior=tail;tail=new Promise(r=>release=r);await prior;const draft=clone(store);let done=false;return {...api(()=>draft),async commit(){if(failCommit){failCommit=false;done=true;release();throw Error('injected commit failure')}store=draft;done=true;release()},async rollback(){if(!done){done=true;release()}}}}
 db.dump=()=>clone(store);db.failNextCommit=()=>{failCommit=true};db.command={in:a=>({$in:a}),all:a=>({$all:a}),or:a=>({$or:a}),gte:a=>({$gte:a}),gt:a=>({$gt:a}),lte:a=>({$lte:a}),lt:a=>({$lt:a})};return db
}
